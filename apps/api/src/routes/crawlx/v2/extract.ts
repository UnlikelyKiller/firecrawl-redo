...
import { ExtractRequestSchema } from "@crawlx/firecrawl-compat";
import {
 ...
 * Handles simple types, objects, and arrays.
 */
...
extractRouter.post("/", async (req, res) => {
  ...
  const { urls, schema: userSchema, model, prompt } = parseResult.data;
  const url = urls[0];
  if (!url) {
    return res
      .status(400)
      .json({ success: false, error: "At least one URL is required" });
  }
  const jobId = uuidv7();
  const orchestratorResult = await buildOrchestrator(url);
  if ("error" in orchestratorResult) {
    return res
      .status(orchestratorResult.status)
      .json({ success: false, error: orchestratorResult.error });
  }
  ...
  if (scrapeResult.isErr()) {
    return res
      ...
      .json({
        success: false,
        error: `Scrape failed: ${scrapeResult.error.message}`,
      });
  }
  ...
  if (!markdown) {
    return res
      .status(500)
      .json({ success: false, error: "No markdown content" });
  }
  ...
  if (extractionResult.isErr()) {
    return res
      ...
      .json({
        success: false,
        error: `Extraction failed: ${extractionResult.error.message}`,
      });
  }
