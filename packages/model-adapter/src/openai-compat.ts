...
export class OpenAICompatAdapter implements ModelAdapter {
  ...
  private async chat(system: string, user: string): Promise<Result<string, AdapterError>> {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 429) {
          return err(new AdapterError('Rate limited by API', 'RATE_LIMITED'));
        }
        return err(new AdapterError(
          `OpenAI-compat HTTP ${res.status}: ${await res.text().catch(() => 'unknown')}`,
      ...
      return err(new AdapterError(
        `OpenAI-compat connection error: ${e instanceof Error ? e.message : String(e)}`,
        'CONNECTION_ERROR',
        e,
      ));
    } finally {
